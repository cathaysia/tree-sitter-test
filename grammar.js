...
module.exports = grammar({
  ...
  externals: $ => [$._equals_begin, $._equals_end, $._dashes],

  extras: _ => [],

  rules: {
    file: $ => repeat(choice($.test, $._eol)),
    ...
    attributes: $ => repeat1(choice($.attribute, $._eol)),
    ...
    custom_attr: $ =>
      seq(
        ':',
        token.immediate(alias(/[\w\-_]+/, $.name)),
        optional(alias(seq('(', repeat(choice(/[^)]/, '\\)')), ')'), $.value)),
      ),
    ...
    _platform: $ =>
      seq(':platform', '(', field('platform', alias($._os, $.parameter)), ')'),
